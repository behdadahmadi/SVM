...
class DecompostionSVM:
    '''
        ###################################################################
            Soft margin Decomposition-SVM, solving with CVXOPT library
        ###################################################################
    ...
    def __init__(self,q,C=0.1,lambda_param=2.4,kernel="poly"):
        ...
        #------specific variables to SVM using Decomposition method------
    ...
    def objective_function(self):
        first_part = 0.5 * (self.alpha.T.dot(self.Q)).dot(self.alpha)
    ...
    def calculate_working_set(self):
        ...
        neg_grady = -1 * (self.Q.dot(self.alpha) - 1)/self.Y #Negative gradient divided by Y(label data)
        ...
        R_q = list(sorted_R.keys())[-self.q//2:]
        ...
        S_q = list(sorted_S.keys())[:self.q//2]
        
        Working_set = R_q + S_q #Working set by adding  R and S sets
        Working_set_not = list(set(np.arange(self.samples)) - set(Working_set)) #Set of not elements in working-set
        ...
        return optimal_solution,Working_set, Working_set_not  
    ...
    def fit(self,X,Y):
        ...
        alpha_old = np.zeros(self.samples)
        
        #self.obj_init = self.objective_function()
        start_time = time()
        for i in range(1000):
            optimal_solution,ws,ws_not = self.calculate_working_set() #Calculating wokring set
            #If optimal solution is found, then it will break the loop
            if not optimal_solution:
                
                P = cvx.matrix(self.Q[np.ix_(ws,ws)])
                q = cvx.matrix(alpha_old[ws_not].T.dot(self.Q[np.ix_(ws_not, ws)]) - 1)
                G = cvx.matrix(np.vstack((-np.eye(len(ws)),np.eye(len(ws)))))
                h = cvx.matrix(np.hstack((np.zeros(len(ws)), np.ones(len(ws)) * self.C)))
                A = cvx.matrix(self.Y[ws].reshape(1, -1))
                b = cvx.matrix(-1* self.Y[ws_not].T.dot(alpha_old[ws_not]))
                ...
                solver_result = cvx.solvers.qp(P, q, G, h, A, b)
                new_alpha = np.array(solver_result['x']).T
                self.n_fev += solver_result["iterations"]

                
                self.alpha[ws] = new_alpha #Setting new alpha
                alpha_old = self.alpha  #Setting old alpha to new alpha
                self.n_iterations +=1 #Counting iterations
